;( function( window ) {
	...
	function SectionSlide(){
		...
		classie.add(this.getCurrentSlide(), 'current-slide');

		var triggers = [].slice.call(document.querySelectorAll('.slide-trigger'));
	...
	SectionSlide.prototype.getCurrentSlide = function(){
	...
	var onOutAnimationEnd = function(){
		this.removeEventListener(animEndEventName, onOutAnimationEnd);
		classie.remove(sectionSlider.getCurrentSlide(), 'current-slide');
		sectionSlider.currentSlide++;
		classie.add(sectionSlider.getCurrentSlide(), 'current-slide');
		sectionSlider.getCurrentSlide().addEventListener( animEndEventName, onInAnimationEnd);
		classie.add(sectionSlider.getCurrentSlide(), 'slideInRight');
	}
	...
	SectionSlide.prototype.moveToNextSlide = function(){
		this.getCurrentSlide().addEventListener( animEndEventName, onOutAnimationEnd);
		classie.add(this.getCurrentSlide(), 'slideOutLeft');
	}

	var sectionSlider = new SectionSlide();
	...
	FForm.prototype._addControls = function() {
		...
		if( this.options.ctrlNavPosition ) {
			this.ctrlFldStatus = this.el.querySelector('.fs-numbers');

			// current field placeholder
			this.ctrlFldStatusCurr = this.ctrlFldStatus.querySelector('.fs-number-current');
			this.ctrlFldStatusCurr.innerHTML = Number( this.current + 1 );
